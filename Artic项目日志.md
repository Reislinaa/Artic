...
| 维度 | 现行值 |
|------|--------|
| 基调 | **纯黑白灰主导**（取消彩色点缀；仅品牌 logo 保留橙色作为唯一记忆点） |
| 点缀色 | 橙已取消；主操作 / 高亮用近黑 `#1A1A1A`～`#0A0A0A`，浅色填充用中性灰 `#EDECE9`，激活留白靠黑白对比 |
| 深色区块 | `#101010` 墨底 + 白字 + **白色中性光晕**（`.ink` / `.ink-glow` / `.ink-horizon`） |
| 背景（浅色区） | 暖米白 `#F5F2EC` |
| 主文字 | 浅色区 `#101010`／深色区 `#FFFFFF` |
| 强调色（浅底） | 用近黑 `#0A0A0A`（取消原橙 `#C2410C`） |
...
### 2026-09-20 · 全局去「AI 味」：改掉一批「AI 生成的通用图标 + AI 腔文案」（用户：这部分 ai 味太浓，AI 就喜欢自己生成的图标加一段 AI 语言，去网上学美术 UI 怎么弄；再看看哪里 ai 味浓就改）

**用户要求**：场景卡这部分「AI 味」太重——典型的「AI 自己生成一个图标 + 一段 AI 语言」；要我上网学真实 UI/美术怎么处理，改造并顺带扫描全站其他 AI 味点。

**诊断（对截图逐卡拆解 + 全站扫描）**，识别出 AI 腔套路：抽象形容词三连（快速/专业/得体、正式/得体/商务）、「一键X为Y」套话、伪口语表演（「脑子里已经想明白」）、功能动词机械并列、万能技术图标 `</>`、冗余「的」字。截图目标即 `/#/features` 的「Use cases」2×3 场景卡（客户沟通/工作汇报/商务邮件/会议记录/跨语言/内容创作）。

**代码改动痕迹**：

1. **`src/pages/IntroPage.jsx`（Use cases 场景卡，主目标）**：
   - 6 条描述全部重写为**具体场景短句**，删掉 AI 腔：如「客户一句语音发来，你回过去的是干净的书面确认。」「会开完了，话也讲清楚了，就卡在把口头话整理成文字。」等；去掉「一键转」「必须」「正式得体」「快速专业得体」「反复打磨」等套路。
   - 把最后一张「内容创作」的万能 `</>` 代码图标换成**笔/编辑图标**；其余描边加粗到 1.7。
   - 卡片加入**编辑式序号行**（左上 01–06 深色编号 + 右上细线图标），去掉通用「灰圆角图标块」，悬停从「上浮+柔和阴影」改为「描边加深+顶部黑线」，贴合黑白细线品牌。
2. **`src/pages/pages.css`**：新增 `.scenario-head`/`.scenario-index`，改写 `.scenario-icon`（去灰底圆角），调整 hover。
3. **`src/components/Features.jsx`（孤儿组件，未被 App 引用，顺手清理）**：副标题「把每一次开口，都变成准确、自然、可直接使用的文字（三连排比）」→「说出口的话，直接成为能发出的文字」；「智能补全/自动润色/跨端同步」描述去掉「快人一步」「无缝同步，处处如一」「张嘴即得成品」等 AI 套话。
4. **`src/pages/IntroPage.jsx` 副标题**：「覆盖高端商务场景最典型的高频沟通与成稿需求」→「从回客户消息到整理会议纪要，开口就成了稿」；hero 副标题去掉「快速、自然、准确地」三连。
5. **`src/pages/DownloadPage.jsx`**：hero 副标题「开始智能输入之旅」→「全平台可用，选好你的设备就开始。」

**验证与部署结果**：`npm run build` ✅（149 modules）。浏览器实测 `/#/features`：6 卡左上 01–06 序号、右上细线图标无灰块、无 `</>` 图标、文案具体场景化、控制台无报错；整体保持黑白克制、无模板感。（注：`Features.jsx` 为未挂载的孤儿组件，改动不影响线上。）已 push `main` 并经 `node scripts/deploy-gh-pages.mjs Reislinaa Artic` 推送 `gh-pages` 更新线上站。

---

### 2026-09-20 · 重建中段「功能展示」：从「7 条左右交替、条条聊天框」改为三种版式交替，简洁但丰富（用户：中段像流水账、单一、全是聊天框，参考输入法/互联网产品如何介绍功能）

**用户要求**：中段太像流水账、太枯燥、「都是单一界面（都是聊天框）」；想要「简洁，但丰富」；参考别的输入法网站与互联网产品网站介绍功能的方式。

**调研（真实参考，未凭空发明）**，用 WebSearch 对比：搜狗/百度/讯飞输入法、Gboard、SwiftKey、Grammarly、DeepL、Otter、Notion、Linear、Figma 等功能页。结论高度一致：
- **只有差异化最强的旗舰能力才配大演示**；其余通用能力降为「图标 + 一句话」卡片/短列表，不占整张聊天框。
- 用 **Bento/不对称网格、Tab 切换、卡片网格、纯文本列表、数字统计穿插** 打破「一条条图文交替」的单一节奏。
- 卡片文案克制：标题 6–12 字、描述 15–25 字，一行内；「简洁但丰富 = 视觉/版式丰富，而非文字堆砌」。

**代码改动痕迹**（`src/components/FeatureShowcase.jsx` + `.css` 全量重写）：

1. **Act 1 · 聚焦大卡（核心能力 01 Speak to Edit）**：唯一保留的大演示（编辑器卡片 + 黑色「改正式一点」命令胶囊），左侧标题/描述 + 一条证据行（3 秒／1 句／所有 三个极简数字），顶部 2px 主色短线标出「核心能力」。
2. **Act 2 · 左交互面板 + 右能力卡**：左侧放可交互的「个性化文风」面板（轻松/正式/亲切 Tab 实时切换）；右侧竖排三枚极简能力卡（02 自我纠正 / 03 自动格式化 / 06 个人词库），每枚 = 细线图标 + 序号 + 标题 + 一句话，不配整图。
3. **Act 3 · 收尾双栏**：左「AI 语音转写」用大数字 3s 撑数据；右「隐私优先」用盾牌 + 三段流程图（语音输入→本地处理→输出文字）。呼应调研里「数字/示意打断纯列表」。
4. 删除旧的 `.feature-band` 交替布局与 7 条逐条 GSAP 入场；三种 Act 统一用既有 `Reveal` 渐入，风格沿用黑白灰、小圆角、细线分层、几乎无阴影。

**验证与部署结果**：`npm run build` ✅（149 modules；CSS 由 64KB 降至 58KB）。静态核验产物包含 `fs-act-core/panel/edge`、新文案「核心能力」「自我纠正识别」，且旧 `.feature-band` 在 JS/CSS 均已移除。浏览器实测：三种 Act 版式明显不同（2 列比例与交互形态各异）、无溢出/重叠/裁切、移动端断点堆叠正常、控制台无 JS 报错。已 push `main` 并经 `node scripts/deploy-gh-pages.mjs Reislinaa Artic` 推送 `gh-pages` 更新线上站。（注：本机未安装 gh CLI，调研走 WebSearch；若需 GitHub 侧内容调研可后续安装 `winget install GitHub.cli`。）
...
**代码改动痕迹**（全站仍走 CSS 变量级联，logo 图片不动，橙色仅 logo 保留作记忆点）：

1. **全局设计令牌**（`src/index.css`）：
   - 点缀色组：`--accent #F59E0B→#1A1A1A`、`--accent-bright→#000`、`--accent-soft→#EDECE9`、`--accent-deep→#0A0A0A`、`--grad-accent→近黑渐变(#2A2A2A→#0A0A0A)`
   - 兼容旧名：`--primary-dark #92400E→#000`、`--grad-soft→rgba(26,26,26,.06)透明`
   - `--shadow-accent` 橙阴影→`rgba(16,16,16,.20)`
   - `::selection` 橙高亮→中性灰 `rgba(26,26,26,.18)`
   - 深色区光斑 `.ink-glow/.blob` 橙色弥散→白色中性光 `rgba(255,255,255,.22→.02)`；`.ink-horizon`→`rgba(255,255,255,.07→.02)`
...
经确认后定下两条方向：
① 核心卖点 = **口语 → 正式书面语**（说「改正式一点」→「该方案具备可行性」）
② **删掉全部跟随鼠标的特效**，保留「产品演示本身在动」，并要求模拟框有
   文字逐字出现（模拟打字 / 转语音）的动画。

**先做诊断，没有直接改配色** —— 查代码确认了四件事：
1. **首屏演示和产品定位自相矛盾（最致命）**：站点标题、副标题、功能区标题都在讲「商务稿」，
   而首屏那张卡演示的是「晚上吃饭吗？」的**约饭聊天**。同一文件里另外 6 个演示卡
   全是商务内容（「明天的会议改成下午三点」「该方案具备可行性」…），只有最显眼的 Hero 是例外。
2. **独家能力与通用功能等权重**：7 条特性版式 / 字号 / 卡片尺寸完全一致，
   「语音编辑」和「隐私优先」长得一模一样 → 滚过去没有落点，这就是「平平无奇」的机制。
3. **最不像卖点的内容占了最大篇幅**：页序是 Hero →「四步开始」（下载/授权/按住说话/享受）
   → 7 个功能 → 六平台支持表。前两者任何输入法都有，却各占一个完整区块。
4. **特效与产品气质冲突**：标题文字随鼠标弥散（逐字被推开 + 模糊）、窗口 3D 倾斜、
   内层视差、光标高光、CTA 磁吸 —— 是「炫技作品集」的语言；文字弥散还直接干扰阅读。

**代码改动痕迹**：

1. **首屏演示重写为「口语 → 商务稿」**（`InputMockup.{jsx,css}` 整体重写）
   - 场景换成商务：「给客户的回复 · 微信」，口语「这个方案我觉得还可以，回头再改改吧」
     → 成稿「该方案具备可行性，建议进一步完善后推进」，并打出「可直接发送」
   - 动效改为状态机 `raw → polishing → out → done` 循环：
     转写逐字出现（64ms/字，麦克风脉冲）→ 停顿 →「正在改写为书面语」→ 成稿逐字输出（48ms/字）
     → 口语原句变灰并加删除线（用「被替换掉」讲故事）→ 停留后循环
   - `prefers-reduced-motion` 下直接给出最终结果，不循环
2. **首屏文案直陈差异**（`Hero.jsx`）：标题 `说出来 / 即成文` → `说出来 / 就是商务稿`；
   副标题改为「不是把语音变成字幕，是直接写出去能发出去的稿」；
   次要按钮 `了解更多` → `看它怎么改稿`
3. **立主次**（`FeatureShowcase.{jsx,css}`）
   - 特性顺序按「独特性」重排：「口语直接改成书面语」提到 **01**，
     转写 / 词库 / 隐私这些通用能力下移到 05–07
   - 新增 `featured` 标记：`01` 带墨黑「核心能力」小标签、标题 24px → 32px、
     文案色加深、内容块上方一条 32×2px 橙色短线（面积可忽略，信号很强）
4. **删掉全部跟随鼠标的特效**
   - 删除 `DisperseText.jsx`（标题文字弥散，5 处引用）与 `MagneticButton.jsx`（磁吸，2 处）
   - `Hero.jsx`：删掉光标驱动的光斑漂移、窗口 3D 倾斜 / 内层视差 / 光标高光，
     以及窗口浮动；只留光斑缓慢「呼吸」一层环境动效
   - `StepsSection.{jsx,css}`：删掉跟随悬停节点的暖色光环；节点悬停不再「放大 + 填满橙色」，
     只加深边框与数字；GSAP 不再给节点填底色（原来会在数字后留一块淡橙方块）
5. **调整首页区块顺序**（`App.jsx`）：`Hero → FeatureShowcase → Steps → Everywhere → Platform`
   先讲「你凭什么不一样」，再讲「怎么开始」
6. **演示卡加宽**（`Hero.css` / `InputMockup.css`）：`.hero-col-right` 是 flex 容器，
   子项默认 shrink-to-fit，实测 490px 的栏里卡片只渲染出 327px；加 `.hero-col-right > * { width: 100% }`
   并把 `.demo` 上限提到 440px
7. 清理死代码：`index.css` 里的 `.dt-char`、`StepsSection.css` 里的 `.steps-halo`

**验证与部署结果**（线上实测）：
- 区块顺序 `home > features > steps > everywhere > platform` ✓
- 鼠标特效残留：`.dt-char` 0 个、`.steps-halo` 0 个、`.hero-window-tilt/glare` 0 个；
  模拟鼠标扫过标题后 `.hero-line` 的 transform 仍为 `none`（**标题不再被推动**）✓
- 演示动画逐字推进：`0/0 → 4/0 → 11/0 → 17/0 → 17/2 → …`；
  四个状态标记依次出现（改写中 / 成稿可见 / 原句划掉 / 「可直接发送」）✓
  麦克风脉冲在转写阶段触发 ✓
- 核心特性：首条带 `feature-band-featured` + 「核心能力」标签，标题 32px vs 其余 24px ✓
- 演示卡宽度 327px → 440px ✓
- 构建通过；lint 无错误
...
| 问题 | 说明 |
|---|---|
| 序号悬空 | `1 2 3 4` 只是小灰字，与上方细线之间有 ~60px 死区，孤零零浮着 |
| **线是断的** | 4 条各自一段短线、彼此留空隙 → 看起来像「线断了」，不像步骤 |
| 没有序列感 | 它是「四步」，但四个块完全平行，读不出先后 |
| 下边缘参差 | 第 3、4 条说明换行成 2 行，1、2 条 1 行 |
| **4 种不同入场动画** | `up / scale / blur / fade` 各弹各的 —— 模板感最重的一点 |
| 零动效 | 滚动无联动、鼠标无反馈 |

**代码改动痕迹**：
- `StepsSection.jsx` 重写
  - 结构：`.steps-track > (.steps-line > .steps-line-fill) + .steps-halo + ol.steps-grid`
  - 数字由 `1` 改 `01`–`04`，并做成**轨道上的圆形节点**
  - **滑动页面动效**：ScrollTrigger `scrub: 0.5` 时间线 —— 橙线 `scaleX 0→1` 生长，
    同时节点（边框 / 文字 / 底色）与文案（`autoAlpha 0.28→1`）按 `stagger` **依次点亮**
  - **随鼠标动效**：暖色光环 `.steps-halo` 沿轨道滑到所悬停的节点
    （`gsap.to(halo, { x })` + `power3.out`，圆心精确对齐节点中心）
  - 用 `gsap.matchMedia('(prefers-reduced-motion: no-preference)')` 包住动画：
    减少动效时**完全不跑 JS 动画**；`gsap.context(…, track)` 限定作用域 + `mm.revert()` 清理
  - 挂载 400ms 后 `ScrollTrigger.refresh()`（字体加载会改变轨道几何位置）
- `StepsSection.css` 重写
  - 贯通细线 + 橙色进度条；**CSS 默认状态即「已填满」** —— JS 未执行时页面依然完整
  - 节点圆点 22px，悬停放大 1.15 并填橙（黑字）
  - `.step-desc { min-height: 3.4em }` 固定两行，消除底部参差
  - ≤992px 隐藏贯通线与光环，改为每格自带上边线（2 列/单列时一条横线不再成立）

**踩到并修掉的一个节奏问题（有实测数据）**：
- 第一版**没写 `duration`**，timeline 总时长被 `stagger` 撑到 ~1.38s，
  而「画线」的 tween 只占前 0.5s（36%）→ **线上实测橙线在约 175px 滚动内就从 0 冲到 1**，
  完全看不到生长过程。
- 改为给每条 tween **显式写 `duration`**（线 = 1，占满整条时间线），
  并把 scrub 区间放宽到 `top 85% → bottom 65%`。
- 修后实测 sweep：`0 → 0 → 0 → 0.46 → 1.0`，画线跨约 400px 滚动，是**可见的渐进过程**。

**验证与部署结果**：
- 构建通过；lint 无错误；`gsap@3.15.0`（`matchMedia` 可用）
- 线上实测：橙线 `scaleX` 随滚动 `0 → 0.34 → 0.46 → 1`；4 个节点依次点亮；
  文案 `autoAlpha` 依次 0.28→1；光环 `opacity=1 / visibility=visible`，
  `x=589` 与第 3 个节点中心一致
- 部署 → https://reislinaa.github.io/Artic/

### 2026-09-10 · v12 重做平台支持表 + 鼠标跟随聚光（用户反馈：设计感很烂、鼠标互动不够好）
**用户要求**：平台支持表这一处「设计感很烂」，并且「鼠标互动动效不够好」；附 `react-bits` 技能。

**关于 react-bits 的取舍（重要）**：
- react-bits 是 **Tailwind v4 + jsrepo** 的组件库，而本项目是**纯 React + 手写 CSS（无 Tailwind）**。
  直接引入其源码会连带 Tailwind 依赖、破坏现有样式体系；
  技能文档本身也写明「当项目无法承载重型动画依赖时不适用」。
- 因此按它的**交互范式**（Spotlight / Glare Hover 一类），
  用已有技术栈（React + CSS 变量 + rAF）**手写同效实现** ——
  这也正是该技能强调的「组件自包含」原则。
...
| 问题 | 实测证据 |
|---|---|
| **表格与页头左边缘错位** | 标题 `left=71px`，表格因 `max-width:900px; margin:0 auto` 被缩进 → 对不齐（**这是「没设计过」的主因**） |
| **一行里夹两个空洞** | 网格 `168px 88px 1fr`：「macOS」只占 ~90px 却给 168px；标签占 ~60px 却给 88px |
| **右侧大片死白** | 说明文字 ~500px，行宽却 900px |
| **6 行完全一样** | 无索引、无重点、无节奏 |
| **图标撑不起锚点** | 20px + 中性灰 |
| **hover 等于没有** | 仅 `rgba(10,10,11,0.022)`（2% 黑） |

**代码改动痕迹**：
- `PlatformSpecs.jsx` 重写
  - 行结构改为 `索引 01–06 → 图标+名称 → 说明 → 状态 → 箭头`
  - **删除 `max-width: 900px; margin: 0 auto`**：表格与页头同一起始线，并吃满容器宽度
  - 标签从中间移到**最右当状态列**（中间不再夹空洞）
  - 每行改为 `<button>`，点击跳下载页 → 悬停反馈有了**真实语义**（不是装饰）
  - 新增**跟随指针的暖色聚光**：`mousemove` 内用 `requestAnimationFrame` 节流，
    坐标写进 CSS 变量 `--mx / --my`，**不触发 React 重渲染**；
    显隐也由 JS 写 `--spot` 而不是 `:hover` —— 指针移出窗口时不会卡在错误状态
- `PlatformSpecs.css` 重写
  - 网格 `44px 210px 1fr 116px 22px`；图标加深到近黑、22px
  - 悬停：行底色 + **索引转橙**（橙色只出现在交互发生那一刻）+ 图标上移微放大 + 箭头滑入
  - `:focus-visible` 有清晰焦点框（键盘可用）
  - 窄屏（≤780px）改为「索引占左侧、名称/说明/状态竖排在第二列」
- `PlatformSection.jsx`：把 `onNavigate` 传给表格

**验证与部署结果**：
- 构建通过；lint 无错误
- 线上实测：标题 `left=71px` / 表格 `left=71px`（**完全对齐**）、表格右边界 1195（吃满宽度）；
  网格 `44px 210px 644px 116px 22px`；行标签为 `BUTTON`
- 聚光实测：注入 mousemove 后 `--mx=319.5px` / `--my=161.5px`，`.pspecs-spot` 计算 `opacity=1`
- 部署 → https://reislinaa.github.io/Artic/
...
**先对齐参考图到底在说什么**（05 节两张参考图）：
- 左「艺术活动官网」：米白底 + 一团**橙黄弥散光斑** + 严格栅格 + 极小的宽字距标签
- 右「WeTransfer」：白底 + 荧光黄光斑 + 巨大字级 + 右下角等宽小字
- 共同点：**浅色底**、黑字、**彩色只来自光斑**、**没有卡片阴影 / 没有大圆角 / 没有居中标题**

**「AI 味」自查（v9 几乎把典型信号做满了）**：

| AI 味信号 | v9 的做法 | v10 的改法 |
|---|---|---|
| 全居中「kicker + 标题 + 副标题」三行套模板 | 每个区块都这样 | **左对齐 + 上方贯通细线 + 极小宽字距小标签** |
| **渐变填充文字** | Hero「即成文」+ 内页标题 `em` | **改纯黑 + 一道橙色下划线**，彩色交给光斑 |
| 胶囊按钮（999px） | 所有按钮 | **8px**（渠道按钮 6px、徽章 4px） |
| 大圆角卡片（20–28px） | 全站 | `--radius 6px` / `-lg 8px` / `-xl 12px` |
| 满地卡片阴影 + hover 上浮 | 默认带阴影、hover 抬 4–6px | **默认无阴影、hover 只加深边框** |
| 深色 Hero + 橙色光晕 | v8/v9 的主视觉 | **改浅色底 + 橙黄弥散光斑** |
| 缩放 / 模糊入场动画 | `rv-scale` / `rv-blur` | 统一为轻微上移淡入 |
| 内页页头背后一团橙色柔光 | 每页都有 | 删除 |

**代码改动痕迹**：
- `src/index.css`（整体重写）：米白 `--paper-2 #F5F2EC` + 橙黄 `--accent #F59E0B`；
  区块标题改 `text-align: left`；`.section-kicker` 加 `padding-top + border-top`（编辑式页头）；
  圆角令牌全面收小；阴影令牌降到几乎不可见；reveal 动画去掉缩放/模糊
- `src/App.css`：按钮去胶囊（8px）、去掉 glow 阴影；卡片**移除默认阴影与 hover 上浮**
- `Hero.css` 重写：浅色底 + 右侧大团弥散光斑（`blur 64px`，边缘完全化开）+ 左下小光斑；
  标签改极小宽字距、无胶囊；**标题改纯黑 + 橙色下划线**；
  速度对比改为**细线分栏**（不再是深色玻璃卡）
- `Navbar.css`：改米白半透明 + 黑字；选中态改墨色下划线；头像改墨底白字
- `Footer.css`：去掉橙色余光，纯黑 + 细线收尾；栏目标题改宽字距大写小字
- `StepsSection.css`：改**目录式栅格**（左对齐 + 每格上方细线 + 极小橙黄编号）
- `FeatureShowcase.css`：`01–07` 序号改中性灰；**7 处橙色气泡/头像/徽章全改墨黑**，
  整个深色区只留麦克风一处橙色；窄屏功能条文案改左对齐
- `pages.css`：内页页头**去居中、去橙色柔光、去渐变文字**
- `InputMockup.css`：气泡圆角 18→10px、输入框去胶囊、阴影减弱

**修复的 2 个真实缺陷**：
1. **导航「登录」与 Hero「了解更多」在浅色底上不可见** —— 它们仍用着深色版的白描边类
   `btn-on-ink`（白字 + 浅底 = 隐形）。已改回 `btn-ghost`。
2. Hero 副标题的 ` · ` 分隔符会落到行首 → 改 `&nbsp;·&nbsp;` 并固定断行

**验证与部署结果**：
- 构建通过；lint 无错误
- 线上实测：hero 底 `rgb(245,242,236)`、导航 `rgba(245,242,236,0.86)`、
  标题 `rgb(16,16,16)`、CTA `rgb(245,158,11)` + `border-radius: 8px`、
  `.section-kicker` 为 `text-align: left` + 1px 上边框、
  功能区标题白 / kicker 与序号 44% 白 / 数据「220」为橙
- 部署 → https://reislinaa.github.io/Artic/

**用户审美偏好（已记入长期记忆，含 v8/v9 被否决的做法）**：
- 要**浅色主调 + 一团弥散彩色光斑**；不要深色 Hero
- 点缀色只给「关键行动 / 核心数据 / 品牌记忆点」，不要铺在标题装饰、序号、下划线、勾选上
- 中性色保持**纯中性**，让点缀色成为唯一暖色（v9 因「底色+灰阶+橙全是暖色」被否）
- 忌：胶囊按钮、大圆角、满地阴影、全居中套模板、渐变文字
